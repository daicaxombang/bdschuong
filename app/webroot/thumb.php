...

{
...

    {
        ...
        if ($count) {
            ...
            $this->query['src'] = dirname($_SERVER['SCRIPT_NAME']) . '/img/' . $img;

            if (!is_file('img/' . $img) || !file_exists('img/' . $img)) {

                $this->query['src'] = file_exists('../../Config/no-image.png') ? dirname(dirname(dirname($_SERVER['SCRIPT_NAME']))) . '/Config/no-image.png' : null;
