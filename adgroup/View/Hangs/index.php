				...
				<tbody>
                   <?php
                        foreach ($view as $key => $value) {
                            ?>
                            <tr>
                                <td><input type="checkbox" name="check[<?php echo $value['Hang']['id'];?>]" /></td>
                                <td><?php echo $key + $this->Paginator->counter('{:start}');?></td>
                                <td>
                                    <?php if($value['Hang']['images']){?>
                                        <img src="<?php echo DOMAIN; ?>img/w25/<?php echo $value['Hang']['images']; ?>" />
                                    <?php }?>
                                </td>
                                <td><a href="<?php echo DOMAINAD.strtolower(basename(dirname(__FILE__)));?>/edit/<?php echo $value['Hang']['id']; ?>" title="Sửa"><?php echo $value['Hang']['name']; ?> </a></td>
                                <td><input name="order[<?php echo $value['Hang']['id'];?>]" type="number" value="<?php echo $value['Hang']['order'];?>" class="text-input" style="width: 44px;" /></td>
                               
                                <td align="center"><?php echo date('d-m-Y h:i:s', strtotime($value['Hang']['modified'])); ?></td>
                                ...
                                <td align="center"><!-- Icons --> 
                                    <a href="<?php echo DOMAINAD.strtolower(basename(dirname(__FILE__)));?>/edit/<?php echo $value['Hang']['id'] ?>" title="Sửa mục này"><img src="<?php echo DOMAINAD ?>img/lotus/icons/pencil.png" alt="Sửa" /></a>
                                    <a href="javascript:confirmDelete('<?php echo DOMAINAD.strtolower(basename(dirname(__FILE__)));?>/delete/<?php echo $value['Hang']['id'] ?>','Xóa mục này sẽ xóa cả các mục con của nó, bạn có chắc chắn muốn xóa không?')" title="Xóa mục này"><img src="<?php echo DOMAINAD ?>img/lotus/icons/cross.png" alt="Xóa" /></a>
                                </td>
                                <td align="center">
                                    <?php if($value['Hang']['status'] == 1) { ?>
                                        <a href="<?php echo DOMAINAD.strtolower(basename(dirname(__FILE__)));?>/close/<?php echo $value['Hang']['id'] ?>" title="Tích vào để không hiển thị mục này"><img src="<?php echo DOMAINAD ?>img/lotus/icons/tick_circle.png" alt="Đã kích hoạt" /></a>
                                    <?php } else { ?>
                                        <a href="<?php echo DOMAINAD.strtolower(basename(dirname(__FILE__)));?>/active/<?php echo $value['Hang']['id'] ?>" title="Tích vào để hiển thị mục này"><img src="<?php echo DOMAINAD ?>img/lotus/icons/cross_circle.png" alt="Chưa kích hoạt" /></a>
                                    <?php } ?>
                                </td>
