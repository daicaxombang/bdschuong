			...
			<table>
                ...
                <tbody>
                    <?php foreach ($view as $key => $value) {?>
                        <tr>
                            <td><input type="checkbox" name="check[<?php echo $value['Exttmp']['id'];?>]" /></td>
                            <td><?php echo $key + $this->Paginator->counter('{:start}'); ?></td>
                            <td><?php echo $value['Exttmp']['name'];?></td>
                            <td><input name="order[<?php echo $value['Exttmp']['id'];?>]" type="number" value="<?php echo $value['Exttmp']['order'];?>" class="text-input" style="width: 44px;" /></td>
                            <td><?php echo date('d-m-Y', strtotime($value['Exttmp']['modified'])); ?></td>
                            ...
                            <td>
                                <a href="<?php echo DOMAINAD.strtolower(basename(dirname(__FILE__)));?>/edit/<?php echo $value['Exttmp']['id'] ?>" title="Sửa mục này"><img src="<?php echo DOMAINAD ?>img/lotus/icons/pencil.png" alt="Sửa" /></a> <a href="javascript:confirmDelete('<?php echo DOMAINAD.strtolower(basename(dirname(__FILE__)));?>/delete/<?php echo $value['Exttmp']['id'] ?>')" title="Xóa mục này"><img src="<?php echo DOMAINAD ?>img/lotus/icons/cross.png" alt="Xóa" /></a>
                            </td>
                            <td>
                                <?php if ($value['Exttmp']['status'] == 0) {?>
                                    <a href="<?php echo DOMAINAD.strtolower(basename(dirname(__FILE__)));?>/active/<?php echo $value['Exttmp']['id']; ?>" title="Kích hoạt" class="icon-5 info-tooltip"><img src="<?php echo DOMAINAD ?>img/lotus/icons/cross_circle.png" alt="Kích hoạt" /></a>
                                <?php } else {?>
                                    <a href="<?php echo DOMAINAD.strtolower(basename(dirname(__FILE__)));?>/close/<?php echo $value['Exttmp']['id']; ?>" title="Đóng" class="icon-4 info-tooltip"><img src="<?php echo DOMAINAD ?>img/lotus/icons/tick_circle.png" alt="Ngắt kích hoạt" /></a>
                                <?php }?>
                            </td>
                            <td><?php echo $value['Exttmp']['id']; ?></td>
                        </tr>
                    <?php } ?>
                </tbody>
                <tfoot>
