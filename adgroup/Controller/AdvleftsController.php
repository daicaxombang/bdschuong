...
class AdvleftsController extends AppController {

    public $name = 'Advlefts';
    public $uses = array('Extention');
    
    public $type = 'advleft';
    public $only_one = false;
    public $img_width = 1000;//set width of image after upload
    ...
    public function add() {
        if (!empty($this->request->data)) {
            ...
            if ($this->Extention->validates()) {
                ...
                if(isset($this->request->data['Extention']['images2'])){
                    $this->Upload->w = $this->img_width;
                    $this->Upload->h = $this->img_height;
                    if(isset($this->request->data['Extention']['name'])) $this->Upload->name = $this->request->data['Extention']['name'];
    				$this->Upload->new = $this->request->data['Extention']['images2'];
    				$this->request->data['Extention']['images2'] = $this->Upload->Process();
                }
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->Extention->validates()) {
                ...
                if(isset($this->request->data['Extention']['images2'])){
                    ...
                    if (!empty($this->request->data['Extention']['images2']['name'])) {
                        $this->Upload->w = $this->img_width;
                        $this->Upload->h = $this->img_height;
                        $this->Upload->delete = $img['Extention']['images2'];
                        if(isset($this->request->data['Extention']['name'])) $this->Upload->name = $this->request->data['Extention']['name'];
                        $this->Upload->new = $this->request->data['Extention']['images2'];
                        $this->request->data['Extention']['images2'] = $this->Upload->Process();
                    }
                    else $this->request->data['Extention']['images2'] = $img['Extention']['images2'];
                }
